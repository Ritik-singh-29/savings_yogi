...
# DATE SELECTION LOGIC
# ==========================================
col_head, col_date, col_download = st.columns([3.2, 1.2, 0.8])
...
# ==========================================
# DOWNLOAD BUTTONS — CSV + FULL PAGE PDF
# ==========================================
# with col_download:
#     st.markdown("<div style='margin-top:25px'></div>", unsafe_allow_html=True)
...
#     # --- Full-page PDF capture via html2canvas + jsPDF ---
#     components.html("""
#         <script src="https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js"></script>
#         <script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js"></script>

#         <button id="pdfBtn" onclick="captureFullPage()" style="
...
#                 // Grab the full Streamlit main content block
...
#                 const pdfWidth  = canvas.width  / 1.8;
#                 const pdfHeight = canvas.height / 1.8;
...
#             } catch(err) {
#                 alert('PDF capture failed: ' + err.message);
#             } finally {
#                 btn.innerText = '⬇ PDF';
#                 btn.disabled = false;
#             }
...
# --- (ALL YOUR IMPORTS & CODE ABOVE REMAINS EXACTLY SAME) ---

# ==========================================
# DOWNLOAD BUTTONS — CSV + FULL PAGE PDF
# ==========================================
with col_download:
    st.markdown("<div style='margin-top:25px' border=1 width=100%></div>" , unsafe_allow_html=True)
    ...
    components.html("""
        ...
        <button id="pdfBtn" onclick="captureFullPage()"> Download PDF</button>
        ...
        async function captureFullPage() {
            ...
            try {
                ...
                // ✅ FIXED TARGET SELECTION (NO NULL ERROR)
                const target =
                ...
                const canvas = await html2canvas(target, {
                    ...
                    // ✅ SAFE WIDTH & HEIGHT
                    width: target.scrollWidth || window.parent.document.body.scrollWidth,
                    height: target.scrollHeight || window.parent.document.body.scrollHeight,
                });
...
# --- (REST OF YOUR FILE CONTINUES EXACTLY SAME, NO CHANGES) ---
# ==========================================
# HEADER RENDERING
# ==========================================
with col_head:
...
with c2:
    ...
    v_data = filtered_data[VIX].dropna()
    if not v_data.empty:
        l = (len(v_data[v_data < 15]) / len(v_data)) * 100
        m = (len(v_data[(v_data >= 15) & (v_data <= 25)]) / len(v_data)) * 100
        h = (len(v_data[v_data > 25]) / len(v_data)) * 100
        st.markdown(f"""<div class="vol-zone low"><div class="vol-title" style="color:#059669;">LOW</div><div class="vol-desc">VIX < 15 (Bullish)</div><div class="vol-pct">{l:.1f}%</div></div><div class="vol-zone mid"><div class="vol-title" style="color:#d97706;">MID</div><div class="vol-desc">VIX 15-25 (Normal)</div><div class="vol-pct">{m:.1f}%</div></div><div class="vol-zone high"><div class="vol-title" style="color:#dc2626;">HIGH</div><div class="vol-desc">VIX > 25 (Fear)</div><div class="vol-pct">{h:.1f}%</div></div>""", unsafe_allow_html=True)
        fig_h = px.histogram(v_data, x=VIX, nbins=30, color_discrete_sequence=['#94a3b8'])
        fig_h.update_layout(plot_bgcolor="white", paper_bgcolor="white", margin=dict(l=0, r=0, t=10, b=0), xaxis=dict(showgrid=False, title=""), yaxis=dict(showgrid=False, showticklabels=False), height=140)
        st.plotly_chart(fig_h, width='stretch', config={'displayModeBar': False})
...
ec, em, ex = get_rate_stats(filtered_data, EFFR)
oc, om, ox = get_rate_stats(filtered_data, OBFR)
sc, sm, sx = get_rate_stats(filtered_data, SOFR)
tc, tm, tx = get_rate_stats(filtered_data, TGCR)
bc, bm, bx = get_rate_stats(filtered_data, BGCR)
